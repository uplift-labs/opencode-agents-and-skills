...
One high-level call should advance as far as safely possible, but only after command arguments are classified and the current available tool list has been checked. The public `autopilot_*` tools below are callable only when they are visible in the current tool list. If `autopilot_intake` is not available for non-empty `/autopilot` argument classification, `autopilot_run_next` is not available for an empty or exact-scope continuation, or `autopilot_status` is not available for required read-only queue inspection, stop and report the missing Autopilot plugin tool surface as a blocker; do not search or scan the repository for CLI/script substitutes, do not use a CLI/script fallback, do not call controller helpers directly, and do not simulate, emulate, or manually mutate plugin-owned ledger/state transitions.

When non-empty `/autopilot` arguments are present and `autopilot_intake` is visible, call it before any claim-capable action. Its output is read-only and uses a separate schema with derived prompt family, recommended workflow, queue summary, resolved scope, `firstTool`, and `firstToolArgs`; do not echo or persist raw prompt text from the command. Follow `firstTool: "autopilot_run_next"` only when that tool is visible and the scope is empty or exact; follow `firstTool: "autopilot_status"` only for read-only free-form queue inventory:

```json
{ "tool": "autopilot_intake", "args": { "argumentsText": "<raw command arguments>" } }
```
...
| `autopilot_intake` | Read-only `/autopilot <arguments>` classifier backed by deterministic prompt-intake code. It returns a separate schema with `schemaVersion`, `intake.category`, prompt family, recommended workflow, queue summary, resolved scope, `firstTool`, `firstToolArgs`, `blockedTool`, and `reason`; it never starts or advances work, and it does not echo or persist raw free-form prompt text by default. |

Expected shared Autopilot tool output for `autopilot_run_next`, `autopilot_status`, `autopilot_collect`, `autopilot_answer_blocker`, and `autopilot_stop`; `autopilot_intake` uses the separate read-only schema described above. Path examples use `<ledgerRoot>`, which defaults to `openspec/changes`. Optional fields such as `taskSummaries[].mrStatus`, `nextActions[].tool`, and `nextActions[].args` may be omitted when they do not apply:
...
Before archive or archive-ready acceptance, require `openspec/changes/<change-id>/automation/retro.json`, root-cause evidence for actionable problems, generated follow-up OpenSpec changes for actionable retrospective findings, and the repository retro gate when available. If a completed change lacks a passed `archiveGate.decision`, `outputs.noFindingsReason` evidence, generated follow-up changes for `target` `project-local` or `opencode-dev-kit`, root-cause/investigation routing for problem entries, or an approved skip with reason and approver, treat the missing JSON retrospective/follow-up as an archive gate blocker and ask only blocker questions returned by the plugin/runtime.

When `npm run openspec:retro-followups -- <change-id>` exists, run it before the gate so actionable `automation/retro.json` problem entries create or update OpenSpec follow-up changes that carry problem, evidence, impact, root cause, recommendation, confidence, and target. Then use `npm run openspec:retro-gate -- <change-id>` as deterministic evidence. If either command fails or is unavailable, report the archive gate status manually instead of mutating protected Autopilot state.
