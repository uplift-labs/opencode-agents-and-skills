...
This skill is an entrypoint, not an implementation skill. It inventories OpenSpec-backed work, chooses the next bounded Universal Development Loop slice, and hands ready task queues to `openspec-autopilot` or broad independent non-Autopilot work to `orchestrator` only when fan-out is safe and useful.
