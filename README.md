...
OpenCode project plugins are loaded from `.opencode/plugins/`. The prototype `openspec-autopilot.ts` plugin exposes the model-facing `autopilot_*` tools, server programmatic trigger hooks, a protected-path guard, and optional TUI command registration. It depends on this repository's TypeScript Autopilot output helper and ledger validator during MVP development.
