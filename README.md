...
- Autopilot escape hatch -> for `ready_runtime_deferred`, `active_change_handoff`, `no_ledgers`, `no_actionable_tasks`, stale evidence, or evidence conflict, do not repeat equivalent no-progress Autopilot calls; hand off to `openspec-apply-change` for `active_change_handoff`, or to `next-step`, manual direct work, `orchestrator`, or a follow-up OpenSpec change according to the state reported by `nextActions[]` and local validation. `no_ledgers` means neither applicable Autopilot ledgers nor unfinished active OpenSpec changes in `tasks.md` were found.
...
- `openspec-autopilot`: agent-oriented OpenSpec Autopilot control plane for ready task ledgers/queues, unfinished active OpenSpec changes in `tasks.md` during explicit `/autopilot` handoff, safe fixed or auto parallel OpenSpec work with plugin/runtime selection evidence, idempotent worker-report collect checks, and strict task-type phases; call `autopilot_run_next` to inspect/continue until blocker, MR wait, limit, `reasonCode: "active_change_handoff"`, or the current MVP `reasonCode: "ready_runtime_deferred"`/no-op boundary. Auto parallel output requires `parallelImplementation.enabled: true`, uses numeric resolved `selection.maxImplementationClaims`, optional `selection.autoDecision`, task-to-`worktreePath` evidence for started streams, and fan-in validation before terminal readiness when multiple tasks start or soft conflicts are accepted. Archive-ready and MR-ready handoffs require the same fan-in evidence through agent/reviewer gates until first-class plugin checks exist. Parallel streams use one owned worktree per stream, MR integration, and post-archive cleanup gated by MR merged plus archive evidence. For `active_change_handoff`, continue the selected change with `openspec-apply-change`; `no_ledgers` means neither applicable ledgers nor unfinished active OpenSpec changes were found.
