...
- `openspec-autopilot`: agent-oriented OpenSpec Autopilot control plane for ready task ledgers/queues, unfinished active OpenSpec changes in `tasks.md` during explicit `/autopilot` handoff, safe parallel OpenSpec work with plugin/runtime selection evidence, idempotent worker-report collect checks, and strict task-type phases; call `autopilot_run_next` to inspect/continue until blocker, MR wait, limit, `reasonCode: "active_change_handoff"`, or the current MVP `reasonCode: "ready_runtime_deferred"`/no-op boundary. For `active_change_handoff`, continue the selected change with `openspec-apply-change`; `no_ledgers` means neither applicable ledgers nor unfinished active OpenSpec changes were found.
