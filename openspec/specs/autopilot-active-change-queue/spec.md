# autopilot-active-change-queue Specification

## Purpose
TBD - created by archiving change enable-autopilot-active-change-queue. Update Purpose after archive.
## Requirements
### Requirement: Active OpenSpec Changes Are Discoverable Without Ledgers

Autopilot SHALL discover unfinished active OpenSpec changes when no applicable plugin-owned task ledger exists, either to report read-only handoff evidence or to feed plugin-owned ledger materialization when a separate materialization contract is active.
...
- **WHEN** read-only status or a materialization-ineligible `autopilot_run_next` path evaluates the queue without an explicit scope
...
#### Scenario: Materialization-capable starts use active-change discovery

- **GIVEN** unfinished active changes exist and no applicable ledger exists
- **AND** a separate plugin-owned materialization contract is active for explicit starts
- **WHEN** a materialization-capable `autopilot_run_next` evaluates the selected active change
- **THEN** active-change discovery supplies deterministic selected-change evidence
- **AND** the output may return materialization evidence instead of `active_change_handoff`
- **AND** worker implementation is not claimed solely because a ledger was created
...
- **GIVEN** Autopilot selected an unfinished active change without a ledger on a read-only or materialization-ineligible path
...
Autopilot SHALL NOT create or mutate protected task ledgers while discovering active OpenSpec changes through read-only status or fallback handoff paths. Plugin-owned materialization-capable starts are governed by the separate materialization contract.
