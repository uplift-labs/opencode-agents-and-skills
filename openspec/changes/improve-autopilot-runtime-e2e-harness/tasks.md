...
- [ ] Add tests proving `autopilot_collect` consumes plugin-owned worker reports, is idempotent across repeated calls, and advances only legal transitions.
...
- [ ] Add tests proving runtime evidence conflicts, including report/task-state mismatches, stop advancement and do not mutate protected state.
...
- [ ] Implement default single-primary Ready-ledger claiming/advancement behavior with observable plugin-owned runtime state or explicitly downgrade claim mode to validation-only dry-run output.
- [ ] Implement worker dispatch/collect placeholders or real worker report ingestion with legal transition checks and consumed-report tracking.
- [ ] Implement blocker question storage and answer validation, or explicitly defer answer recording while keeping unknown-question rejection.
- [ ] Implement evidence-conflict classification before claim, dispatch, collect, or transition writes, including report/task-state mismatch handling; update the public output contract if a new reason code is introduced.
...
- [ ] Add claim -> status/stop continuity tests or revise output wording so `advanced`/`tasksStarted` cannot imply persisted plugin-owned state when no active claim is observable.
- [ ] Add repeated collect tests proving worker reports are consumed or reported as already handled, not advanced repeatedly.
- [ ] Harden parallel implementation guard evidence for worktree/lock ownership or rescope current behavior as deterministic harness simulation only.
- [ ] Reconcile blocker-answer design with implementation: either record accepted answers in plugin-owned state or document validation-only accepted answers as deferred persistence.
- [ ] Re-run relevant reviewer gates after runtime scope is fixed or formally downgraded.
