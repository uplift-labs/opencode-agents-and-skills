...
- [ ] Add `tools/test-autopilot-write-gate.ts` covering no active lock, active main-session block, active worker scoped allow, active worker out-of-scope block, inactive worker block, protected-path block, corrupt runtime fail-closed, read-only validation allow, and unknown tool block.
...
- [ ] Add source-equivalent plugin hook smoke tests proving a main session cannot mutate ordinary repository files while an active Autopilot lock exists.
- [ ] Add source-equivalent plugin hook smoke tests proving a plugin-owned worker session can mutate only `scope.write` while `running` and is blocked after `collecting`, `blocked`, `waiting_mr`, `stopped`, `failed`, or `done`.
...
- [ ] Add `autopilot_status` and `autopilot:check` tests for compact lock evidence, corrupt/stale lock diagnostics, and no prompt/report leakage.

## Implementation

- [ ] Implement `tools/autopilot-write-gate.ts` as a pure deterministic decision helper that composes tool classification, runtime lock evidence, protected-path policy, and existing worker-scope guard results.
...
- [ ] Update `.opencode/plugins/openspec-autopilot.ts` `tool.execute.before` to enforce the write gate for active locks and corrupt runtime state.
- [ ] Keep existing protected-path guard behavior always-on for `.autopilot/**` and `openspec/changes/*/automation/**` regardless of active lock state.
- [ ] Add fail-closed classification for mutating shell commands while preserving explicit read-only validation command allowlisting.
- [ ] Ensure worker sessions are recognized only from plugin-owned runtime evidence and only while run status is `running`.
- [ ] Add `writeGate` options with safe defaults for explicit Autopilot and compatibility mode for protect-state-only installs.
- [ ] Extend `autopilot_status` output with compact write-gate/lock summaries and safe next actions.
- [ ] Extend `npm run autopilot:check` with lock consistency checks and blocking diagnostics for corrupt or contradictory active lock evidence.

## Documentation And Routing

- [ ] Update `.opencode/skills/openspec-autopilot/SKILL.md` so agents treat active write-gate blocks as authoritative and do not continue with manual implementation under Autopilot.
...
- [ ] Update `README.md` Autopilot runtime guidance with write-gate modes, active-lock recovery, restart requirements, and single-runtime boundary.
- [ ] Update `openspec/project.md` validation guidance if `autopilot:check` gains new lock consistency gates.
- [ ] Update drift/contract tests for new option names, status fields, reason wording, and protected write-gate behavior.
...
- [ ] `npm run autopilot:check -- --level standard --change add-autopilot-fail-closed-write-gate`.
- [ ] Source-equivalent plugin smoke proving main-session ordinary file mutation is blocked under active Autopilot lock.
- [ ] Source-equivalent plugin smoke proving worker scoped mutation remains allowed only while actively `running`.
