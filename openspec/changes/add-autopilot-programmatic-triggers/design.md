...
- `.opencode/plugins/openspec-autopilot.ts` exposes only model-facing tools: `autopilot_run_next`, `autopilot_status`, `autopilot_collect`, `autopilot_answer_blocker`, and `autopilot_stop`.
...
type AutopilotTriggerOptions = {
  ...
  postToolCheckpoints?: { enabled?: boolean; debounceMs?: number };
  ...
  runNextEvents?: { enabled?: boolean; requireActiveRun?: boolean; cooldownMs?: number };
};
```
...
- `openspec/changes/*/automation/task.json`: schedule ledger validation/status; never edit the ledger from the event path.
...
4. `retry` or `session.error` records a failed-worker signal and schedules status/stop handling instead of collect unless a valid report exists.
...
`permission.replied` should update only plugin-owned pending actions. A reject reply should surface a blocker or stop recommendation. An allow reply may schedule status and, only under explicit policy, a scoped continuation.
