...
Autopilot SHALL route model-facing tools, event-driven actions, trigger-hook actions, and future TUI/SDK actions through a shared controller that preserves the public Autopilot output contract.
...
- **GIVEN** a supported event schedules an Autopilot action
...
- **GIVEN** several `file.watcher.updated` events arrive for the same active OpenSpec `tasks.md` or Autopilot ledger path within the debounce window
...
#### Scenario: Autopilot ledger changes

- **GIVEN** `file.watcher.updated` reports a change to `openspec/changes/<change>/automation/task.json`
- **WHEN** observe-mode triggers are enabled
- **THEN** Autopilot schedules ledger validation or status for that ledger
- **AND** it does not mutate the ledger or advance runtime state from the file event path
...
- **WHEN** plugin-owned worker, blocker, permission, workspace, or failure events occur
