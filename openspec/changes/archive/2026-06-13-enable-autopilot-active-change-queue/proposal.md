...
- Add a read-only active OpenSpec change queue fallback for Autopilot when no applicable task ledger exists; plugin-owned ledger materialization is owned by the separate materialization contract.
...
- Return a distinct reason code, task/actionability summaries, selection evidence, and next action for active-change handoff instead of `no_ledgers` on read-only or materialization-ineligible paths; materialization-capable starts may return `ledger_materialized` under the separate materialization change.
...
- Do not generate, edit, or migrate `automation/task.json` ledgers in this change's read-only discovery fallback; plugin-owned materialization is specified separately and must remain controller-owned.
...
- `/autopilot` will no longer report `no_ledgers` when unfinished active OpenSpec changes are available; it may hand off read-only or materialize a plugin-owned ledger when the materialization contract is active.
