...
Implement a read-only active-change fallback queue. The fallback is not a protected ledger writer and does not pretend to perform plugin-owned worker dispatch. It gives the model enough structured evidence to hand off to `openspec-apply-change` for the deterministic selected change, or to feed a separate plugin-owned materialization path when that contract is active.
...
Add a reason code such as `active_change_handoff` to mean: no applicable ledger-backed task can be claimed and the current path is read-only or materialization-ineligible, but at least one unfinished active OpenSpec change exists and should be continued by the agent through the OpenSpec apply workflow. A materialization-capable `run_next` path may instead create a plugin-owned ledger and return `ledger_materialized`; that behavior is owned by the materialization contract and still depends on this change's active-change discovery evidence.
...
- Read-only active-change discovery/status must not write `.autopilot/**`, `openspec/changes/*/automation/**`, or any protected ledger path. Plugin-owned materialization-capable starts are specified separately and remain the only allowed writer for generated `automation/task.json` ledgers.
