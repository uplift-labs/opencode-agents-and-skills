...
- Implement controlled triggers for plugin-owned worker session idle events, blocker question replies, permission replies, and workspace/worktree ready or failed signals only when plugin-owned runtime evidence ties the event to an Autopilot run; defer richer `session.error`/retry worker-failure handling to a follow-up spec.
...
| --- | --- | --- |
| `file.watcher.updated` for `tasks.md`, `automation/**`, reports, or retrospectives | observe | Debounced status or cheap check only |
| `tool.execute.after` for `autopilot_run_next` or `autopilot_collect` | observe | Cheap checkpoint after progress; status checkpoint after runtime evidence conflict |
...
| `permission.replied` for plugin-owned pending actions | controlled | MVP status-only evidence; richer blocked/stop handling requires a follow-up spec |
